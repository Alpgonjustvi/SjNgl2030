      ...
      try {
        ...
        messages.forEach(async msg => {
          ...
          svg1.addEventListener("click", () => {
            window.location.href = `/block_user#${msg.fingerprint}?${msg._id}`;
          ...
          li.style.display = msg.event ? "none" : "flex";
  ...
  if (data.count >= 1) {
    ...
    document.querySelector("#toggleSwitch").addEventListener("change", event => {
      if (event.target.checked) {
        ...
        document.querySelector("h3").innerText = "Event Answers To Saint Joseph 2030"
        document.querySelector(".color_paletteUl").style.display = "none"
        Array.from(document.querySelector("ul#messages").children).forEach(li => {
          if (li.id) {
            li.style.display = "flex"
          } else {
        ...
        document.querySelector("#ngl_bg_top").style.background = "linear-gradient(135deg, rgb(95, 192, 209), rgb(190, 190, 208))";
        document.querySelector("#ngl_bg_top h3").innerText = data.event.name

      } else {
        document.body.style.background = "linear-gradient(135deg, rgb(65, 65, 189), rgb(148, 148, 189))";
        document.querySelector("h3").innerText = "NGL's To Saint Joseph 2030"
        document.querySelector(".color_paletteUl").style.display = ""
        Array.from(document.querySelector("ul#messages").children).forEach(li => {
          if (!li.id) {
            li.style.display = "flex"
          } else {
        ...
        document.querySelector("#ngl_bg_top").style.background = "linear-gradient(to bottom right, rgb(239, 40, 113), rgb(239, 60, 60), rgb(254, 135, 21))"
        document.querySelector("#ngl_bg_top h3").innerText = "bana anonim olarak mesajlar gönder!"
      }
    });
