...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    <span style="display: flex; align-items: center; gap: 5px; padding-top: 5px">
      ...
      document.querySelector("#background").style.minHeight = "max-content"
      ...
      document.querySelector("#toggleSwitch").addEventListener("change", event => {
        if (event.target.checked) {
          ...
          url.searchParams.set("event", data1.event._id);
          window.history.pushState({}, "", url); // changes URL bar but doesn’t reload
          ...
          const url = new URL(window.location.href);
          url.searchParams.delete("event"); // ?event parametresini komple kaldırır
          window.history.pushState({}, "", url); // sayfa reload olmadan URL güncellenir          
        }
      })
    }
  ...
  document.getElementById("send_btn").addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        document.querySelector("#background span").remove();
