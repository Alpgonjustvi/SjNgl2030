...
router.patch('/sendMessage', async (req, res) => {
  try {
    const { ngl, fingerprint, eventId } = req.body;
...
router.patch('/getEvent', async (req, res) => {
  try {
    ...
    if (!event) return res.status(404).json({ error: "event didn't exist" });

    res.json({ success: true, event }); // event bilgisini JSON olarak dön
  } catch (err) {
...
router.get('/messages', async (req, res) => {
  try {
    ...
    // Önce blocked = false olanlar, sonra blocked = true olanlar, her grupta createdAt'a göre sıralı ///IPTAL
